...
public final class P38SectionSyntheticClockReplay {
    ...
    private record PassResult(String name,int snapshots,int uniqueLanes,int firstPropSeenMs,int firstPropRunningMs,
                              double gainPositiveLaneSeconds,double propGainPositiveSeconds,
                              double runningLaneSeconds,double propRunningLaneSeconds,
    ...
    public static void main(String[] args) throws Exception {
        ...
        try(FileWriter fw=new FileWriter(args[1])){
            fw.write("pass,snapshots,unique_lanes,first_prop_seen_ms,first_prop_running_ms,gain_positive_lane_seconds,prop_gain_positive_seconds,running_lane_seconds,prop_running_lane_seconds,baseline_entries,running_entries,recipes\n");
            for(PassResult r:List.of(cold.result(),warm.result()))
                fw.write(String.format(Locale.US,"%s,%d,%d,%d,%d,%.1f,%.1f,%.1f,%.1f,%d,%d,%d\n",
                        r.name,r.snapshots,r.uniqueLanes,r.firstPropSeenMs,r.firstPropRunningMs,
                        r.gainPositiveLaneSeconds,r.propGainPositiveSeconds,r.runningLaneSeconds,r.propRunningLaneSeconds,
                        r.baselineEntries,r.runningEntries,r.recipes));
        }
    ...
    private static Outcome run(String name,float[] source,List<VehicleCancellationRecipe> recipes){
        ...
        Map<String,String> previousStage=new HashMap<>(); Set<String> seenIds=new HashSet<>();
        int snapshots=0,firstPropSeen=-1,firstPropRunning=-1,baselineEntries=0,runningEntries=0;
        double gainPositiveLaneSeconds=0,propGainPositiveSeconds=0,runningLaneSeconds=0,propRunningLaneSeconds=0;
        for(int i=0;i<source.length;i++){
            ...
            for(VehicleLaneRegistry.Lane lane:VehicleLaneRegistry.lanes()){
                ...
                boolean running="RUNNING".equals(lane.stage());
                if(prop&&firstPropSeen<0)firstPropSeen=relMs;
                if(lane.gain()>1e-5){gainPositiveLaneSeconds+=TICK_SECONDS;if(prop)propGainPositiveSeconds+=TICK_SECONDS;}
                if(running){runningLaneSeconds+=TICK_SECONDS;if(prop)propRunningLaneSeconds+=TICK_SECONDS;}
                if(prop&&running&&firstPropRunning<0)firstPropRunning=relMs;
                String prior=previousStage.put(lane.id(),lane.stage());
                if("BASELINE".equals(lane.stage())&&!"BASELINE".equals(prior))baselineEntries++;
                if(running&&!"RUNNING".equals(prior))runningEntries++;
            }
        }
        List<VehicleCancellationRecipe> learned=new ArrayList<>(bank.drainLearnedRecipes());
        return new Outcome(new PassResult(name,snapshots,seenIds.size(),firstPropSeen,firstPropRunning,
                gainPositiveLaneSeconds,propGainPositiveSeconds,runningLaneSeconds,propRunningLaneSeconds,
                baselineEntries,runningEntries,learned.size()),learned);
    }
