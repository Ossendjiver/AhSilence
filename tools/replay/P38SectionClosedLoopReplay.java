...
public final class P38SectionClosedLoopReplay {
    ...
    private static final int DELAY=2400; // 50 ms unity secondary path
    ...
    public static void main(String[] args) throws Exception {
        ...
        float[] source=readSection(wav,20.0,200.0);
        List<VehicleCancellationRecipe> none=List.of();
        PassResult cold=run("cold",source,none);
        // The production bank only exposes learned observations through drainLearnedRecipes().
        // Re-run cold internally once more to retrieve them without changing the reported cold metrics.
        Learned learned=runForRecipes(source);
        PassResult warm=run("warm",source,learned.recipes);
        try(FileWriter fw=new FileWriter(args[1])){
            ...
            for(PassResult r:List.of(cold,warm))
    ...
    private static final class Learned { final List<VehicleCancellationRecipe> recipes; Learned(List<VehicleCancellationRecipe> r){recipes=r;} }

    private static Learned runForRecipes(float[] source) throws Exception {
        ...
        for(int i=0;i<source.length;i++){
            ...
            if((i+1)%4800==0) pace(start,(i+1)/(double)SR);
        }
        return new Learned(new ArrayList<>(bank.drainLearnedRecipes()));
    }

    private static PassResult run(String name,float[] source,List<VehicleCancellationRecipe> recipes) throws Exception {
        VehicleLaneRegistry.clear();
        VehicleNarrowbandBank bank=new VehicleNarrowbandBank(List.<MechanicalFrequency>of(),null,0.08f,0.50f,
        ...
        int learned=bank.drainLearnedRecipes().size();
        return new PassResult(name,snapshots,admissions,firstPropAdmission,firstPropRunning,
                activeLaneSeconds,propLaneSeconds,baselineEntries,runningEntries,learned);
    }
    ...
    private static void pace(long startNanos,double targetSeconds) throws InterruptedException {
        ...
        while(true){
            long left=target-System.nanoTime();
            if(left<=0)return;
            long ms=left/1_000_000L;int ns=(int)(left%1_000_000L);
            Thread.sleep(ms,ns);
        }
    }
    ...
    private static float[] readSection(File file,double startSeconds,double endSeconds) throws Exception {
        try(AudioInputStream in=AudioSystem.getAudioInputStream(file)){
            ...
            long skipFrames=(long)(startSeconds*SR);long bytes=skipFrames*2;
            ...
            for(int i=0;i<out.length;i++){
                int lo=raw[2*i]&0xff;int hi=raw[2*i+1];short s=(short)((hi<<8)|lo);out[i]=s/32768f;
            }
            return out;
        }
    }
