...
public final class MainActivity extends Activity {
    ...
    private View buildInterface(){
        ...
        TextView subtitle=text("v0.5.x · predictive headphones + measured-error vehicle broadband",11,MUTED);subtitle.setPadding(dp(18),0,dp(18),dp(10));shell.addView(subtitle);
    ...
    private ScrollView buildControlPage(){
        ...
        root.addView(section("LIVE SESSION"));LinearLayout live=card(CARD_ALT,Color.rgb(42,62,79));runtimeText=text("Stopped · output muted",14,GREEN);runtimeText.setTypeface(Typeface.DEFAULT,Typeface.BOLD);live.addView(runtimeText);live.addView(text("Headphones use the phone microphone as external reference. Vehicle broadband uses the selected cabin microphone as measured error sensor and reconstructed reference. Predictable configured vehicle tones are notched out of the speculative broadband path so the dedicated narrowband architecture remains separate.",11,MUTED),topSpaced());root.addView(live,spaced());
    ...
    private ScrollView buildSplPage(){
        ...
        LinearLayout meter=card(Color.rgb(13,33,37),Color.rgb(30,89,94));meter.addView(text("LIVE MICROPHONE LEVEL",13,MUTED));meter.addView(text("Reference/error RMS is shown in the live status. Vehicle broadband uses this same selected microphone to measure whether cancellation actually changes cabin sound.",12,TEXT),topSpaced());root.addView(meter,spaced());
    ...
    private void updateProfileUi(){
        ...
        if(broadbandText!=null){broadbandText.setVisibility(h?View.GONE:View.VISIBLE);if(!h)broadbandText.setText("Optional 15–600 Hz measured-error feedback FxNLMS. Configured predictable engine/shaft frequencies are notched out before this controller. Toggle can be changed while vehicle ANC is running.");}
        if(profileWarning!=null){if(h){profileWarning.setText("HEADPHONES · predictive 15–600 Hz feed-forward FxNLMS · disconnect protection active");profileWarning.setTextColor(CYAN);}else{profileWarning.setText((ProfileStore.PROFILE_E46.equals(currentProfile)?"E46":"P38")+" · vehicle route uses selected microphone as error sensor. Speculative broadband is optional and excludes configured predictable-frequency bands.");profileWarning.setTextColor(AMBER);}}
    ...
    private void startAnc(){
        ...
        boolean ok=isHeadphones()?rt.audio.startHeadphoneAnc():rt.audio.startVehicleAnc(currentProfile,broadbandCheck!=null&&broadbandCheck.isChecked(),profiles.loadPredictableFrequencies(currentProfile));
