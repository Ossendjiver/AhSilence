...
public final class MainActivity extends Activity {
    ...
    private Spinner profileSpinner,inputSpinner,outputSpinner,settingsInputSpinner,settingsOutputSpinner,toneLabFrequencySpinner;
    ...
    private ScrollView buildControlPage(){
        ...
        root.addView(section("ANC CONTROL"));LinearLayout control=card(CARD,Color.TRANSPARENT);profileWarning=text("",12,AMBER);control.addView(profileWarning);broadbandCheck=check("SPECULATIVE BROADBAND ANC",false);control.addView(broadbandCheck,topSpaced());broadbandText=text("ⓘ",18,CYAN);broadbandText.setOnClickListener(v->showInfo("Broadband ANC",String.valueOf(v.getTag())));control.addView(broadbandText,topSpaced());startButton=primaryButton("START ANC");startButton.setOnClickListener(v->toggleAnc());control.addView(startButton,topSpaced());toneLabFrequencySpinner=new Spinner(this);ArrayAdapter<String> ta=new ArrayAdapter<>(this,android.R.layout.simple_spinner_item,new String[]{"45 Hz","65 Hz","90 Hz","120 Hz","160 Hz","250 Hz","500 Hz"});ta.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);toneLabFrequencySpinner.setAdapter(ta);toneLabFrequencySpinner.setSelection(3);control.addView(toneLabFrequencySpinner,topSpaced());toneLabButton=secondaryButton("RUN FIXED-TONE CANCELLATION LAB");toneLabButton.setOnClickListener(v->toggleToneLab());control.addView(toneLabButton,topSpaced());graphButton=secondaryButton("OPEN LIVE WAVE GRAPH");graphButton.setOnClickListener(v->startActivity(new Intent(this,GraphActivity.class)));control.addView(graphButton,topSpaced());root.addView(control,spaced());
    ...
    private void updateProfileUi(){
        ...
        if(graphButton!=null)graphButton.setEnabled(true);if(toneLabButton!=null){toneLabButton.setText(rt.audio.isFixedToneLab()?"STOP FIXED-TONE LAB":"RUN FIXED-TONE CANCELLATION LAB");toneLabButton.setEnabled(!rt.audio.isRunning()||rt.audio.isFixedToneLab());}
    ...
    private final Runnable statusTick=new Runnable(){@Override public void run(){
        if(runtimeText!=null){String safety=rt.audio.getSafetyStatus();String session=rt.audio.isFixedToneLab()?String.format(Locale.US,"%.0f Hz cancellation lab",rt.audio.getFixedToneLabFrequencyHz()):profileName();runtimeText.setText(String.format(Locale.US,"%s · %s · mic %.5f RMS · drive %.5f RMS · limit %d%%%s",rt.audio.isRunning()?"RUNNING":"STOPPED",session,rt.audio.getInputRms(),rt.audio.getOutputRms(),rt.audio.getAntiNoisePercent(),safety==null||safety.isEmpty()?"":"\n"+safety));}
        ...
        if(startButton!=null)startButton.setText(rt.audio.isRunning()?"STOP ANC":isHeadphones()?"START HEADPHONE ANC":isRoom()?"START ROOM ANC":"START VEHICLE ANC");if(toneLabButton!=null){toneLabButton.setText(rt.audio.isFixedToneLab()?"STOP FIXED-TONE LAB":"RUN FIXED-TONE CANCELLATION LAB");toneLabButton.setEnabled(!rt.audio.isRunning()||rt.audio.isFixedToneLab());}handler.postDelayed(this,400);
    }};
