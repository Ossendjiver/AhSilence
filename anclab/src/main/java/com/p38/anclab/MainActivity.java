...
public final class MainActivity extends Activity {
    ...
    private Button controlTab,splTab,settingsTab,startButton,recordButton,calibrateButton,useStoredButton,graphButton,splButton,testCycleButton,outputSweepButton;
    private TextView storageText,calibrationText,runtimeText,profileWarning,recordingText,testCycleText,antiNoiseText,broadbandText,laneCountText,laneListText,splValueText,splStatsText,micCalibrationText,micBenchmarkText,outputCapabilityText,obdStatusText,bluetoothWarningText,settingsBluetoothWarningText;
    private Spinner profileSpinner,inputSpinner,outputSpinner,settingsInputSpinner,settingsOutputSpinner,obdSpinner;
    private CheckBox positionCheck,outputSweepCheck,monitorLogCheck,broadbandCheck,androidAutoCheck,keepScreenCheck,autoStartCheck;
    ...
    @Override protected void onCreate(Bundle b){
        ...
        rt=AncRuntime.get(this);profiles=new ProfileStore(rt.storage);appSettings=new AppSettings(this);benchmarkReferenceSpl=appSettings.benchmarkReferenceSpl();if(rt.storage.isConnected())profiles.ensureDefaults();
    ...
    private View buildInterface(){
        ...
        TextView subtitle=text("v0.5.9.7 recovery",11,MUTED);subtitle.setPadding(dp(18),0,dp(18),dp(10));shell.addView(subtitle);
    ...
    private ScrollView buildSplPage(){
        ...
        root.addView(section("MICROPHONE CALIBRATION"));LinearLayout mic=card(CARD,Color.TRANSPARENT);micCalibrationText=text("No calibration for selected microphone",13,MUTED);mic.addView(micCalibrationText);micBenchmarkText=text("Select and start the calibrated reference microphone",12,MUTED);mic.addView(micBenchmarkText,topSpaced());Button importCal=secondaryButton("IMPORT FREQUENCY RESPONSE");importCal.setOnClickListener(v->pickMicCalibration());mic.addView(importCal,topSpaced());Button absoluteCal=secondaryButton("SET ABSOLUTE SPL REFERENCE");absoluteCal.setOnClickListener(v->promptAbsoluteSplCalibration());mic.addView(absoluteCal,topSpaced());Button captureReference=secondaryButton("1 · CAPTURE CALIBRATED REFERENCE");captureReference.setOnClickListener(v->captureBenchmark(true));mic.addView(captureReference,topSpaced());Button benchmark=secondaryButton("2 · CALIBRATE CURRENT / PHONE MIC");benchmark.setOnClickListener(v->captureBenchmark(false));mic.addView(benchmark,topSpaced());mic.addView(infoIcon("Microphone calibration","Keep one steady sound playing. Capture the calibrated UMIK first, then select the phone microphone and capture again. The reference survives input switching and app recreation for ten minutes."),topSpaced());root.addView(mic,spaced());
    ...
    private ScrollView buildSettingsPage(){
        ...
        LinearLayout capability=card(CARD,Color.TRANSPARENT);outputCapabilityText=text("No output capability sweep",13,MUTED);capability.addView(outputCapabilityText);outputSweepCheck=check("SAFE SWEEP POSITION CONFIRMED",false);capability.addView(outputSweepCheck,topSpaced());outputSweepButton=secondaryButton("RUN 20–200 HZ OUTPUT SWEEP");outputSweepButton.setOnClickListener(v->runOutputSweep());capability.addView(outputSweepButton,topSpaced());root.addView(collapsibleSection("OUTPUT CAPABILITY","Measures the selected physical output twice through the selected microphone. Unsupported frequencies are monitored but cannot emit anti-noise.",capability,false),spaced());
    ...
    private void runOutputSweep(){
        if(!rt.storage.isConnected()){toast("Connect Documents/ANC first");return;}if(outputSweepCheck==null||!outputSweepCheck.isChecked()){toast("Confirm the safe sweep position first");return;}if(!hasAudioPermission()){requestAudioPermissionIfNeeded();return;}
        if(outputSweepButton!=null)outputSweepButton.setEnabled(false);if(outputCapabilityText!=null){outputCapabilityText.setText("Starting two-pass output sweep…");outputCapabilityText.setTextColor(AMBER);}
        new Thread(()->{AudioEngine.OutputSweepResult result=rt.audio.measureOutputCapability();runOnUiThread(()->{if(outputSweepButton!=null)outputSweepButton.setEnabled(true);refreshOutputCapabilityUi();toast(result.message);});},"ANC-Output-Sweep").start();
    }
    ...
    private void applyMicCalibrationForRoute(){rt.audio.reloadMicrophoneCalibration();MicCalibration calibration=rt.audio.getMicrophoneCalibration();if(micCalibrationText!=null){if(calibration==null){micCalibrationText.setText("No calibration · "+rt.audio.getResolvedInputRoute());micCalibrationText.setTextColor(AMBER);}else{String state=(calibration.hasFrequencyResponse()?"response":"")+(calibration.hasFrequencyResponse()&&calibration.hasSplOffset()?" + ":"")+(calibration.hasSplOffset()?"absolute SPL":"");micCalibrationText.setText(calibration.description()+" · "+state+" · "+rt.audio.getResolvedInputRoute());micCalibrationText.setTextColor(calibration.hasSplOffset()?GREEN:AMBER);}}refreshMicBenchmarkUi();}

    private void refreshMicBenchmarkUi(){if(micBenchmarkText==null)return;long age=System.currentTimeMillis()-appSettings.benchmarkReferenceUtcMs();if(!Double.isFinite(benchmarkReferenceSpl)||age<0||age>600000L){micBenchmarkText.setText("Step 1: select and start the calibrated reference microphone");micBenchmarkText.setTextColor(MUTED);}else{micBenchmarkText.setText(String.format(Locale.US,"Reference ready: %.1f dB SPL · %s\nStep 2: select the phone mic and capture",benchmarkReferenceSpl,appSettings.benchmarkReferenceRoute()));micBenchmarkText.setTextColor(GREEN);}}

    private void refreshOutputCapabilityUi(){if(outputCapabilityText==null)return;outputCapabilityText.setText(rt.audio.getOutputSweepStatus()+"\n"+rt.audio.getResolvedOutputRoute());outputCapabilityText.setTextColor(rt.audio.getOutputCapability()==null?AMBER:GREEN);}

    private void captureBenchmark(boolean reference){if(!rt.audio.isSplMeterEnabled()){if(!hasAudioPermission()){requestAudioPermissionIfNeeded();return;}if(!rt.audio.startSplMeter()){toast("Could not start microphone capture: "+rt.audio.getLastError());return;}applyMicCalibrationForRoute();startMonitorService();updateSplUi();}if(!requirePhysicalInput())return;if(reference&&!rt.audio.isSplCalibrated()){toast("Set an absolute SPL reference for this calibrated microphone first");return;}if(!reference){long age=System.currentTimeMillis()-appSettings.benchmarkReferenceUtcMs();if(!Double.isFinite(benchmarkReferenceSpl)||age<0||age>600000L){toast("Reference expired · capture the calibrated microphone again");benchmarkReferenceSpl=Double.NaN;refreshMicBenchmarkUi();return;}if(rt.audio.getInputCalibrationKey().equals(appSettings.benchmarkReferenceKey())){toast("Select the phone microphone before step 2");return;}}new Thread(()->{double measuredDbFs=captureAverageDbFs();if(!Double.isFinite(measuredDbFs)){runOnUiThread(()->toast("No microphone level captured"));return;}runOnUiThread(()->{if(reference){benchmarkReferenceSpl=measuredDbFs+rt.audio.getSplOffsetDb();appSettings.setBenchmarkReference(benchmarkReferenceSpl,rt.audio.getInputCalibrationKey(),rt.audio.getResolvedInputRoute());refreshMicBenchmarkUi();toast("Reference captured · now select the phone microphone");}else{if(profiles.saveMicSplCalibration(rt.audio.getInputCalibrationKey(),rt.audio.getResolvedInputRoute(),benchmarkReferenceSpl-measuredDbFs)){applyMicCalibrationForRoute();toast("Phone microphone benchmark saved");}else toast("Could not save the phone microphone benchmark");}});},"ANC-Mic-Benchmark").start();}
    ...
    private final Runnable statusTick=new Runnable(){@Override public void run(){
        ...
        if(outputSweepButton!=null&&!outputSweepButton.isEnabled()&&outputCapabilityText!=null)outputCapabilityText.setText(rt.audio.getOutputSweepStatus());
