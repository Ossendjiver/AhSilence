...
public final class MainActivity extends Activity {
    ...
    private View buildInterface(){
        ...
        TextView subtitle=text("v0.6.2 development · headphones + vehicle + direct-feedback room ANC",11,MUTED);subtitle.setPadding(dp(18),0,dp(18),dp(10));shell.addView(subtitle);
    ...
    private ScrollView buildSplPage(){
        ...
        LinearLayout meter=card(Color.rgb(13,33,37),Color.rgb(30,89,94));meter.addView(text("LIVE MICROPHONE LEVEL",13,MUTED));meter.addView(text("Reference/error RMS is shown in the live status. Vehicle mode verifies narrowband cancellation here; Room mode treats this microphone as the directly observed error sensor and repeatedly feeds successful path corrections into its recipe book.",12,TEXT),topSpaced());root.addView(meter,spaced());
    ...
    private ScrollView buildSettingsPage(){
        ...
        root.addView(section("ROUTE CALIBRATION"));LinearLayout bench=card(CARD,Color.TRANSPARENT);positionCheck=check("I have positioned the microphone/output safely for a low-level route probe",false);bench.addView(positionCheck);calibrationText=text("No stored calibration",13,MUTED);bench.addView(calibrationText,topSpaced());calibrateButton=secondaryButton("RUN / RE-RUN ROUTE CALIBRATION");calibrateButton.setOnClickListener(v->runCalibration());bench.addView(calibrateButton,topSpaced());useStoredButton=secondaryButton("USE STORED CALIBRATION");useStoredButton.setOnClickListener(v->loadSavedCalibration());bench.addView(useStoredButton,topSpaced());bench.addView(text("Headphones: place the IEMs beside the phone microphone only for calibration. Vehicle: put the selected cabin microphone at the listening position. Room: keep the error microphone fixed at the listening position where it will hear the ANC result. Calibration also records current media-volume gain for ANC transport compensation.",11,MUTED),topSpaced());root.addView(bench,spaced());
        root.addView(section("SENSORS + MULTI-CHANNEL ANC"));LinearLayout sensors=card(CARD,Color.TRANSPARENT);sensors.addView(text("Configure future ADXL345 transmission-tunnel references, left/right footwell input microphones, B-pillar error microphones, stereo output channels and later C-pillar / wheel-arch / floor sensors. Sensor-primary control is enabled only after both sides have calibrated reference, output and error paths; existing ANC remains independent and becomes secondary/fallback when that occurs.",12,TEXT));Button sensorSettings=secondaryButton("OPEN SENSOR + MULTI-CHANNEL SETTINGS");sensorSettings.setOnClickListener(v->{Intent i=new Intent(this,SensorSettingsActivity.class);i.putExtra("profile",currentProfile);startActivity(i);});sensors.addView(sensorSettings,topSpaced());root.addView(sensors,spaced());
