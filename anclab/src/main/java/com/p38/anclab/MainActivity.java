...
public final class MainActivity extends Activity {
    ...
    private void buildUi(){
        ...
        TextView title=text("ANC LAB",28,Color.WHITE);title.setLetterSpacing(.12f);root.addView(title);root.addView(text("v0.5.0 · Headphones broadband reference-mic FxNLMS",12,muted()));space(14);

        cardHeader("USER DATA");storageText=text("Storage not connected",13,muted());root.addView(storageText);Button storageButton=button("CONNECT / RESELECT  Documents/ANC");storageButton.setOnClickListener(v->startActivityForResult(rt.storage.createTreePickerIntent(),REQ_TREE));root.addView(storageButton);root.addView(text("Profiles, calibrations, logs and WAV files remain in the shared Internal storage/Documents/ANC hierarchy.",12,muted()));space(14);
        ...
        cardHeader("INPUT + ROUTING");root.addView(text("Reference microphone",12,muted()));inputSpinner=new Spinner(this);root.addView(inputSpinner,full());root.addView(text("Cancellation output",12,muted()));outputSpinner=new Spinner(this);root.addView(outputSpinner,full());Button refresh=button("REFRESH INPUTS + OUTPUTS");refresh.setOnClickListener(v->refreshDevices());root.addView(refresh);space(14);

        cardHeader("HEADPHONE BENCH");headphoneWarning=text("",12,warn());root.addView(headphoneWarning);positionCheck=new CheckBox(this);positionCheck.setText("Calibration only: headphones are beside the phone microphone and not being worn");positionCheck.setTextColor(Color.WHITE);root.addView(positionCheck);calibrationText=text("No stored calibration",13,muted());root.addView(calibrationText);calibrateButton=button("RUN / RE-RUN HEADPHONE CALIBRATION");calibrateButton.setOnClickListener(v->runCalibration());root.addView(calibrateButton);useStoredButton=button("USE STORED CALIBRATION");useStoredButton.setOnClickListener(v->loadSavedCalibration());root.addView(useStoredButton);root.addView(text("Runtime: phone microphone is an external reference only. 128-tap feed-forward FxNLMS uses the stored headphone secondary-path model; no in-ear residual is measured.",12,muted()));startButton=button("START HEADPHONE ANC");startButton.setOnClickListener(v->toggleAnc());root.addView(startButton);graphButton=button("OPEN LIVE WAVE GRAPH");graphButton.setOnClickListener(v->startActivity(new Intent(this,GraphActivity.class)));root.addView(graphButton);space(8);

        monitorLogCheck=new CheckBox(this);monitorLogCheck.setText("BACKGROUND MONITOR LOG");monitorLogCheck.setChecked(true);monitorLogCheck.setTextColor(Color.WHITE);monitorLogCheck.setOnCheckedChangeListener((b,c)->rt.audio.setMonitorLogEnabled(c));root.addView(monitorLogCheck);recordButton=button("START WAV + CSV LOG");recordButton.setOnClickListener(v->toggleRecording());root.addView(recordButton);runtimeText=text("Stopped · output muted",12,muted());root.addView(runtimeText);space(14);

        cardHeader("P38 / VEHICLE PROFILE");root.addView(text("The shared Documents/ANC profile/log structure is retained. This branch changes the Headphones path only; reconstructed vehicle ANC remains disabled while validation continues.",12,muted()));Button share=button("SHARE PROFILE FOR CHATGPT REVIEW");share.setOnClickListener(v->shareSummary());root.addView(share);
    }

    private void updateProfileUi(){if(headphones){calibrateButton.setEnabled(true);useStoredButton.setEnabled(true);startButton.setEnabled(true);graphButton.setEnabled(true);headphoneWarning.setText("HEADPHONE BENCH · during normal IEM use the phone mic hears the environment, not the cancellation at your ear. Calibration is a separate bench measurement of the headphone→phone-mic path.");}else{calibrateButton.setEnabled(false);useStoredButton.setEnabled(false);startButton.setEnabled(false);graphButton.setEnabled(false);headphoneWarning.setText("P38 profile selected · headphone controller disabled.");if(rt.audio.isRunning())stopAnc();}}
    ...
    private final Runnable statusTick=new Runnable(){@Override public void run(){runtimeText.setText(String.format(Locale.US,"%s · reference %.5f RMS · drive %.5f RMS · %s",rt.audio.isRunning()?"RUNNING":"STOPPED",rt.audio.getInputRms(),rt.audio.getOutputRms(),rt.audio.getLastError()));handler.postDelayed(this,500);}};
