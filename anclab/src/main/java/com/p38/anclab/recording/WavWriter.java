...
/** 32-bit IEEE-float WAV writer so quiet input and anti-noise remain measurable in diagnostics. */
public final class WavWriter implements AutoCloseable {
    ...
    public synchronized void writeMonoPair(float[] input,float[] output,int n)throws IOException{byte[] b=new byte[n*channels*4];int q=0;for(int i=0;i<n;i++){q=putFloat(b,q,input[i]);if(channels>1)q=putFloat(b,q,output[i]);}out.write(b);dataBytes+=b.length;}
    ...
    @Override public synchronized void close()throws IOException{out.flush();out.close();try(RandomAccessFile r=new RandomAccessFile(file,"rw")){int byteRate=sampleRate*channels*4;r.seek(0);r.writeBytes("RIFF");writeLe32(r,(int)(36+dataBytes));r.writeBytes("WAVE");r.writeBytes("fmt ");writeLe32(r,16);writeLe16(r,3);writeLe16(r,channels);writeLe32(r,sampleRate);writeLe32(r,byteRate);writeLe16(r,channels*4);writeLe16(r,32);r.writeBytes("data");writeLe32(r,(int)dataBytes);}}
